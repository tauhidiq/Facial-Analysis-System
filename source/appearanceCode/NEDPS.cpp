...
NEDPS::NEDPS()
{
	mConfigFile = "NEDPS.conf";
	std::map<std::string, kifas::NumStr> nedpsconfig;
	kifas::ConfigFileManager::ConfigFileRead(mConfigFile, nedpsconfig);

	edgepercent = nedpsconfig["edgepercent"];
	scorethresh = nedpsconfig["scorethresh"];
	sigma = nedpsconfig["sigmaw"];
	ignorecode = nedpsconfig["ignorecode"];
	padding = nedpsconfig["padding"];
}

std::vector<cv::Mat> NEDPS::runCodeGenerate(const std::vector<cv::Mat> &input)
{
	...
	addWeighted(abs_grad_x, 0.5f, abs_grad_y, 0.5f, 0, MAGNITUDE);*/
	...
	/*cv::imwrite("D:\\Research\\FAS_GLR\\bin\\sourceImage.png", sourceImage);
	cv::imwrite("D:\\Research\\FAS_GLR\\bin\\mag.png", MAGNITUDE);*/
	...
	for (p = 0; p < dy; p++)
		for (q = 0; q < dx; q++)
		{
			...
			{
				/*edgeImg.at<uchar>(cvPoint(q, p)) = 255;*/
	...
	cv::imwrite("D://Research//1.PAPER//NEDP_TAC//NEDPS_uneven//file.png", dstCode);

	//cv::imwrite("D:\\Research\\FAS_GLR\\bin\\edge.png", edgeImg);
...
double* NEDPS::gaussianTable(int radius) {
...
int NEDPS::angleDiff(int position, float angle) {
...
int NEDPS::calcSIGN(int position, float angle) {
...
int NEDPS::ComputeMagThreshold(cv::Mat gradMag, float mT1Percent)
{
